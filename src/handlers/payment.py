...
async def choose_rate(callback: CallbackQuery):
    ...
    await callback.message.answer(
        ...
        + f"\n<b>Комиссия при переходе на данный тариф {tax} руб</b>",
...
async def input_balance(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "Введите сумму через панель или вручную."
        f"<b>\n(Актуальная комиссия провайдера {settings.transfer_fee*100-100} %</b>"
    )
    await callback.message.answer("100", reply_markup=pay_keyboard)
    await state.set_state(Service.balance)


@router.callback_query(F.data.startswith("balance_"))
async def change_balance(callback: CallbackQuery, bot: Bot):
    process: str = callback.data.split("_")[-1]

    if process.isnumeric():
        with suppress(TelegramBadRequest):
            await callback.message.edit_text(
                callback.message.text + process, reply_markup=pay_keyboard
            )
    elif process == "incr":
        with suppress(TelegramBadRequest):
            await callback.message.edit_text(
                callback.message.text[:-1], reply_markup=pay_keyboard
            )
    else:
        with suppress(TelegramBadRequest):
...
async def pay(trigger: Union[Message, CallbackQuery], bot: Bot, state: FSMContext):
    try:
        ...
        await getattr(trigger, "message", trigger).answer(
            hlink(
                f"Многоразовая ссылка на пополнение счета на {get_plural(SUM, 'рубль, рубля, рублей')} (Ссылка №2)",
